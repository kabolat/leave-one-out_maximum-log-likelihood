...
class GaussianMixtureTorch(torch.nn.Module):
    """
    ...
    """
    def __init__(self, n_components, n_features, covariance_type="full", eps=1.e-4, init_params="kmeans", mu_init=None, var_init=None, verbose=False):
        """
    ...
    def fit(self, x, delta=1e-4, n_iter=1000000, warm_start=False):
        """
        ...
        while (i <= n_iter) and (j >= delta):
            ...
            if torch.isinf(self.log_likelihood.abs()) or torch.isnan(self.log_likelihood):
                raise ValueError("Log-likelihood is inf or nan")
